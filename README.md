...
|---|---|---|
| `similarity_threshold` | 0.5 | min cosine for a match; below → unknown (calibrated on production calls: true matches 0.5–0.8, wrong-person 0.3–0.45) |
| `merge_threshold` | 0.6 | stricter bar for merges/dedup (destructive ops) |
| `snr_tiers` | 20/15/10 dB | progressive SNR relaxation ladder (quality labels) |
| `min_kept_speech_sec` | 15 s | rescue target: top up with best remaining segments when the tiers keep less speech than this (telephony audio reads 5–15 dB on WADA) |
| `min_segment_confidence` | 0.5 | ElevenLabs-confidence gate (hard) |
| `min_segment_sec` / `max_segment_sec` | 1 / 10 s | utterance segment bounds |
| `enroll_outlier_margin` | 0.3 | drop enrollment segments far below the batch-median agreement (diarization misattribution guard) |
| `max_embeddings_per_speaker` | 100 | per-speaker cap (quality-based eviction) |
